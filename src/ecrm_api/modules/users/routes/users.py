...
from ecrm_api.modules.users.presenters.users import UserCreate, UserUpdate, ChangePasswordSchema

from ecrm_api.modules.users.services.users import new, get_all, delete, update, get_one_by_id, change_password
...
async def get_user(request: Request, page: int = 1, per_page: int = 6, search: str = "", db: Session = Depends(get_db)):
...
async def update_user(request: Request, user_id: str, user: UserUpdate, db: Session = Depends(get_db)):
...
@users_router.post("/password", response_model=BaseResult, summary="Change password to a user.")
def reset_password(
    request: Request,
